...
// form `@media (width<=640px)` unless build.cssTarget constrains it. Browsers
// older than Chrome 104 / Safari 16.4 cannot parse the range form and discard
// the entire at-rule, silently disabling every responsive rule in the bundle.
// This guard fails the build so that regression can never ship unnoticed.

import { readdirSync, readFileSync } from 'node:fs';
import { join } from 'node:path';

const RANGE_SYNTAX = /@media[^{]*\((?:width|height)\s*[<>]=?/;
